...
const Navbar = ({classContainer, openModal, sobre}) => {
	...
	return (
		<div
			className={`w-full z-[99999999] shadow-sm fixed top-0 bg-white h-[5rem] grid place-items-center ${classContainer}`}>
			<header className="max-w-[1100px] relative w-[1100px] md2:w-[90vw] items-center flex justify-between">
				<img
					src={logo}
					className="w-[80px] md2:w-[70px] cursor-pointer"
					onClick={() => {
						navigate('/home');
					}}
				/>
				<SvgUse
					onClick={() => openModal('sidemenu')}
					id="#icon_hamburguer_ms"
					classe="hidden md2:block h-32 w-32"
				/>
				<nav className="flex items-center md2:hidden">
					<ul className="flex gap-40">
						{!sobre && (
							<li className="paragraph2 cursor-pointer">
								<a href="#beneficios">Benefícios de contratar</a>
							</li>
						)}
						{!sobre && (
							<li
								className="paragraph2 relative cursor-pointer"
								onClick={() => {
						...
						<li
							className="paragraph2 cursor-pointer"
