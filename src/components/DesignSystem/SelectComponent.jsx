...
//Simple
//state setstate
const SelectComponentReceptor = ({ data, classe, classeSelect, placeholder, value, state, setStateCurrent, ...opts }, ref) => {
...
const SelectGroup = ({ data, classe }) => {
	...
	return (
		<div className={`select-container ${classe}`}>
			<Select  className={`select-container select-multiple `} classNamePrefix="select" options={data} isMulti closeMenuOnSelect={false} onChange={(e) => setSelectValue(e)} defaultValue={[...selectValue]} />
		</div>
	)
}
