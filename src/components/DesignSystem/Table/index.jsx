...
const Table = ({ options, arrData, arrColumns, recentes, setRecentes, pageLimit, searchExternal, classeTableContainer, HeaderCustom }) => {
	...
	return (
		<>
			<div className="flex flex-col">
				{
					opts.headerChildren ? <HeaderCustom filter={globalFilter} setFilter={setGlobalFilter} pageSize={pageSize} setPageSize={setPageSize} /> :
						opts.headerWithTable.enabled
							? <HeaderTableWithTitle opts={opts} setRecentes={setRecentes} setPageSize={setPageSize} recentes={recentes} />
							:
							opts.headerCustom
								? opts.headerCustom()
								: <div className={`flex w-full gap-32 mb-24 md2:flex-col md2:gap-10 ${opts.searchResponsive}`}>
									{opts.search && (
									...
									)}
									{
										opts.customInputDate.enabled && (
											<label className={`inline-flex gap-8  select-none ${opts.customInputDate.class}`}>
												{opts.customInputDate.title && <p className="text-sm font-medium text-neutral-80 md2:hidden">
									...
									{opts.itemsLength && opts.linhasPorPagina?.state && (
										<label className={`inline-flex gap-8 items-center select-none md2:justify-end ${opts.viewPerPageResponsive}`}>
								...
								{headerGroups.map((headerGroup) => (
									<tr {...headerGroup.getHeaderGroupProps()}>
										{headerGroup.headers.map((column, i) => (
											<th
												className={`${opts.classTHS} ${column.classTH ? column.classTH : ""}`}
												key={i}
												data-id={i}
												{...!column?.noFilter ? { ...column.getHeaderProps(column.getSortByToggleProps()) } : ""}
											>
								...
								{page.map((row) => {
									...
									return (
										<tr {...row.getRowProps()}>
											{row.cells.map((cell) => {
												return (
													<td className={`${opts.classTDS}`} {...cell.getCellProps()}>{cell.render("Cell")}</td>
												);
											})}
										</tr>
									);
				...
				{
					data.length > 0 && (
						<div className={`flex gap-16 w-full ${!opts.notPagination ? "mt-16" : ''} justify-end`}>
							{opts.download && (
								<Button classe="secondary mr-auto h-48" iconID={"#icon_excel_ms"}>
									Baixar tabela
								</Button>
							)}
									...
									<div className="ml-auto md:ml-0 flex gap-8 justify-start table-pagination select-none ">
										...
										<div className="flex items-center gap-8 ">
											...
											<div className={`${!canNextPage ? "disabled" : ""}`}>
												<button className="btn rounded-btn " onClick={() => nextPage()}>
...
export const HeaderTableWithTitle = ({ opts, setRecentes, recentes, pageSize, setPageSize }) => {
	...
	return (
		<div className={`flex w-full gap-32 mb-24 md2:flex-col md2:gap-10 ${opts.searchResponsive}`}>
			...
			{/* LINHYAS POR PAGINA */}
			{opts.headerWithTable.linhasPorPagina && opts.headerWithTable.linhasPorPagina.state && (
				<label className={`inline-flex gap-8 items-center select-none md2:justify-end ${opts.viewPerPageResponsive}`}>
					{opts.linhasPorPagina.layout === 'left' && (
						<>
							<p className="text-sm font-medium text-neutral-80">
